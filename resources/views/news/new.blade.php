      ...
      <div class="col-lg-10">
        <div class="panel panel-info">
          <div class="panel-heading">Catégorie : {{ $news->categorie }}
             @if ($user->id == $news->user_id) <a href="news/{{$news->id}}/delete" button class="btn btn-danger btn-small pull-right">Supprimer</a>@endif
             @if ($user->id == $news->user_id) <a href="news/{{$news->id}}/edit" button class="btn btn-success btn-small pull-right">Editer</a>@endif
            ...
            <img src="{{ asset('image/' . $news->picture)}}"/>
              ...
              <h4>Pseudo : {{ $user->pseudo}}, {{ $user->lastname }}</h4>
              <h4>Poster le : {{ $news->created_at}}</h4>
